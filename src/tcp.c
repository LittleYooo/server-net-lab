...
void tcp_out(tcp_conn_t *tcp_conn, buf_t *buf, uint16_t src_port, uint8_t *dst_ip, uint16_t dst_port, uint8_t flags) {
    ...
    tcp_hdr->doff = ((TCP_HEADER_LEN / 4) << 4) | 0x0;
    tcp_hdr->flags = flags;
    tcp_hdr->win = swap16(TCP_MAX_WINDOW_SIZE);
    tcp_hdr->checksum16 = 0;
    tcp_hdr->uptr = 0;

    tcp_hdr->checksum16 = swap16(transport_checksum(NET_PROTOCOL_TCP, buf, net_if_ip, dst_ip));
...
void tcp_in(buf_t *buf, uint8_t *src_ip) {
    ...
    // 校验checksum
    uint16_t checksum = swap16(hdr->checksum16);
    hdr->checksum16 = 0;
    ...
    if (buf->len > tcp_hdr_sz) {
        ...
        if (tcp_handler != NULL) {
            (*tcp_handler)(tcp_conn, buf->data + tcp_hdr_sz, buf->len - tcp_hdr_sz, src_ip, remote_port);
        } else {
    ...
    buf_t tx_buf;
    buf_init(&tx_buf, 0);
    tcp_out(tcp_conn, &tx_buf, host_port, remote_ip, remote_port, send_flags);
    // TODO: 更新序列号
